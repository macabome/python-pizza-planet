...
def __order(ingredients: list, size: dict, client_data: dict, beverages: list):
    ingredients = [_ingredient.get('_id') for _ingredient in ingredients]
    beverages = [_beverage.get('_id') for _beverage in beverages]
    ...
    for param, value in order.items():
        ...
        for item in created_order['detail']:
            if item['ingredient'] is not None:
                ingredients.append(item['ingredient']['_id'])
            if item['beverage'] is not None:
                beverages.append(item['beverage']['_id'])
        ingredients_in_detail = set(ingredients)
        pytest.assume(not ingredients_in_detail.difference(ingredient_ids))
        beverages_in_detail = set(beverages)
...
def test_get_all(app, ingredients, sizes, client_data, beverages):
    ...
    created_orders = []
    for _ in range(5):
        order = __order(shuffle_list(created_ingredients)[:3], get_random_choice(created_sizes), client_data, shuffle_list(created_ingredients)[:3])
        created_order, _ = OrderController.create(order)
